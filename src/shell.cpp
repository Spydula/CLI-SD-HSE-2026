...
void Environment::set(std::string name, std::string value) { vars_[std::move(name)] = std::move(value); }
...
std::map<std::string, std::string> Environment::snapshot() const { return vars_; }
...
static bool isValidVarName(std::string_view name) {
  ...
  if (!isAlphaUnd(static_cast<unsigned char>(name[0]))) return false;
  for (size_t i = 1; i < name.size(); ++i) {
    if (!isAlnumUnd(static_cast<unsigned char>(name[i]))) return false;
  }
  return true;
}

static std::string errnoMessage() { return std::system_error(errno, std::generic_category()).what(); }
...
int Shell::run(std::istream& in, std::ostream& out, std::ostream& err) {
  ...
  while (true) {
    ...
    if (r.shouldExit) return r.exitCode;
  }
  return 0;
}
...
ExecResult Shell::executeLine(std::string_view line, std::ostream& out, std::ostream& err) {
  ...
  if (argv.empty()) return ExecResult{0, false};
...
Environment& Shell::env() { return env_; }
const Environment& Shell::env() const { return env_; }
...
ExecResult Shell::executeArgv(const std::vector<std::string>& argv, std::ostream& out, std::ostream& err) {
  const std::string& cmd = argv[0];

  if (cmd == "cat") return builtinCat(argv, out, err);
  if (cmd == "echo") return builtinEcho(argv, out, err);
  if (cmd == "wc") return builtinWc(argv, out, err);
  if (cmd == "pwd") return builtinPwd(out, err);
  if (cmd == "exit") return builtinExit();
...
ExecResult Shell::builtinExit() { return ExecResult{0, true}; }
...
ExecResult Shell::builtinEcho(const std::vector<std::string>& argv, std::ostream& out, std::ostream& err) {
  (void)err;
...
ExecResult Shell::builtinCat(const std::vector<std::string>& argv, std::ostream& out, std::ostream& err) {
  if (argv.size() != 2) {
...
ExecResult Shell::builtinWc(const std::vector<std::string>& argv, std::ostream& out, std::ostream& err) {
  if (argv.size() != 2) {
...
ExecResult Shell::runExternal(const std::vector<std::string>& argv, std::ostream& err) {
  ...
  for (auto& s : argsStorage) args.push_back(s.data());
