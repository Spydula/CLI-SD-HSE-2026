# CLI-SD-HSE-2026
Учебный проект: Интерпретатор командной строки на C++17 с поддержкой кавычек, переменных окружения, подстановок `$`, пайплайнов и запуска внешних программ.
...
- `grep` — поиск строк по регулярному выражению с поддержкой ключей:
...
## 5. Библиотеки для разбора аргументов командной строки

Для реализации команды grep была выбрана стандартная библиотека std::regex для работы с регулярными выражениями, а также стандартные средства C++ для разбора аргументов командной строки.

Вместо использования сторонних библиотек, таких как Apache Commons CLI или boost::program_options, мы использовали:
1. Стандартную библиотеку C++ для регулярных выражений (std::regex)
2. Простой разбор аргументов командной строки вручную, поскольку количество ключей ограничено и не требует сложной системы парсинга

Этот подход позволяет избежать зависимостей от сторонних библиотек и сохраняет совместимость.
